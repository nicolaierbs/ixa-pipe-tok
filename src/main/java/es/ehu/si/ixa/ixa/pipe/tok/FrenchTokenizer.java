...
import es.ehu.si.ixa.ixa.pipe.tok.jflex.French;


/**
 *  FrenchTokenizer is based on the {@link French} class. 
 *  This Tokenizer overrides {@link AbstractTokenizer} getToken() method 
 *  by using the {@link French} yylex() method.  
 *  
 ...
 * French Corpus for French. Many of the normalization rules have been adapted from 
 * the @link PTBLexer class of Stanford CoreNLP version 3.2.0, but with many changes for 
 * other requirements, such as those of the French corpus. 
 *  
 * Specifically, apart from English Penn Treebank-compliant tokenization, 
 * this Tokenizer provides:
 *  
 * <ol>
 * <li> multilingual treatment of apostrophes for Catalan, French styles 
 ...
 *       acronyms and person titles, etc., for French. 
 *  <li> normalization for French Treebank corpus.
 ...
 *   <li> French: French corpus based normalization. Like default, except that every 
 *        quote is normalized into ascii quotes. 
 * </ol> 
 * 
 * The normalization performed by the four options above are (in the order in which
 * they appear in the @link French specification):
 * <ol> 
...
public class FrenchTokenizer<T> extends AbstractTokenizer<T> {

  
  private French jlexer;
  
  /**
   * Construct a new Tokenizer which uses the @link French specification
   * 
   * 
   * @param breader Reader
   * @param tokenFactory The TokenFactory that will be invoked to convert
   *        each string extracted by the @link French  into a @Token object
   * @param properties the properties to run the tokenizer
   * 
   */
  public FrenchTokenizer(BufferedReader breader, TokenFactory tokenFactory, Properties properties) {
    jlexer = new French(breader, tokenFactory, properties);
  }

  /**
   * It obtains the next token. This functions performs the actual tokenization 
   * by calling the @link French yylex() function
   *
