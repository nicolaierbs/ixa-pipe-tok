...
import es.ehu.si.ixa.ixa.pipe.tok.jflex.Ancora;


/**
 *  AncoraTokenizer is based on the {@link Ancora} class. 
 *  This Tokenizer overrides {@link AbstractTokenizer} getToken() method 
 *  by using the {@code Ancora} yylex() method.  
 *  
 ...
 *       acronyms and person titles, etc., for Spanish. 
 ...
 * they appear in the @link Ancora specification):
 * <ol> 
...
public class AncoraTokenizer<T> extends AbstractTokenizer<T> {

  
  private Ancora jlexer;
  
  /**
   * Construct a new Tokenizer which uses the @link Ancora specification
   * 
   * 
   * @param breader Reader
   * @param tokenFactory The TokenFactory that will be invoked to convert
   *        each string extracted by the @link Ancora  into a @Token object
   * @param properties the properties to run the tokenizer
   * 
   */
  public AncoraTokenizer(BufferedReader breader, TokenFactory tokenFactory, Properties properties) {
    jlexer = new Ancora(breader, tokenFactory, properties);
  }

  /**
   * It obtains the next token. This functions performs the actual tokenization 
   * by calling the @link Ancora yylex() function
   *
