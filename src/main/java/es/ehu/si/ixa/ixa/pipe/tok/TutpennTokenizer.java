...
import es.ehu.si.ixa.ixa.pipe.tok.jflex.Tutpenn;


/**
 *  TutpennTokenizer is based on the {@link Tutpenn} class. 
 *  This Tokenizer overrides {@link AbstractTokenizer} getToken() method 
 *  by using the {@link Tutpenn} yylex() method.  
 *  
 ...
 * Tutpenn Corpus for Italian. Many of the normalization rules have been adapted from 
 * the @link PTBLexer class of Stanford CoreNLP version 3.2.0, but with many changes for 
 * other requirements, such as those of the Tutpenn corpus. 
 *  
 ...
 *       acronyms and person titles, etc., for Italian. 
 *  <li> normalization for Tutpenn corpus in Italian.
 ...
 *   <li> Tutpenn: Tutpenn corpus based normalization. Like default, except that every 
 *        quote is normalized into ascii quotes. 
 * </ol> 
 * 
 * The normalization performed by the four options above are (in the order in which
 * they appear in the @link Tutpenn specification):
 * <ol> 
...
public class TutpennTokenizer<T> extends AbstractTokenizer<T> {

  
  private Tutpenn jlexer;
  
  /**
   * Construct a new Tokenizer which uses the @link Tutpenn specification
   * 
   * 
   * @param breader Reader
   * @param tokenFactory The TokenFactory that will be invoked to convert
   *        each string extracted by the @link Tutpenn  into a @Token object
   * @param properties the properties to run the tokenizer
   * 
   */
  public TutpennTokenizer(BufferedReader breader, TokenFactory tokenFactory, Properties properties) {
    jlexer = new Tutpenn(breader, tokenFactory, properties);
  }

  /**
   * It obtains the next token. This functions performs the actual tokenization 
   * by calling the @link Tutpenn yylex() function
   *
