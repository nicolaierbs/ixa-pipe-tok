...
/**
 *  IxaPipeTokenizer is based on the {@link English} class. 
 *  This Tokenizer overrides {@link AbstractTokenizer} getToken() method 
 *  by using the {@link English} yylex() method.  
 *  
 ...
 * they appear in the @link English specification):
 * <ol> 
...
public class EnglishTokenizer<T> extends AbstractTokenizer<T> {

  
  private English jlexer;
   ...
   *        each string extracted by the @link English  into a @Token object
   * @param properties the properties to run the tokenizer
   * 
   */
  public EnglishTokenizer(BufferedReader breader, TokenFactory tokenFactory, Properties properties) {
    jlexer = new English(breader, tokenFactory, properties);
  }

  /**
   * It obtains the next token. This functions performs the actual tokenization 
   * by calling the @link English yylex() function
   *
