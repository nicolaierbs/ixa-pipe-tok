...
import es.ehu.si.ixa.ixa.pipe.tok.jflex.Alpino;


/**
 *  AlpinoTokenizer is based on the {@link Alpino} class. 
 *  This Tokenizer overrides {@link AbstractTokenizer} getToken() method 
 *  by using the {@link Alpino} yylex() method.  
 *  
 ...
 * Alpino Corpus for Dutch. Most of the normalization rules have been adapted from 
 * the @link PTBLexer class of Stanford CoreNLP version 3.2.0, but with many changes for 
 * other requirements, such as those of the Alpino corpus. 
 *  
 ...
 *       acronyms and person titles, etc., for Dutch. 
 *  <li> normalization for Alpino corpus in Spanish.
 ...
 *   <li> Alpino: Alpino corpus based normalization. Like default, except that every 
 *        quote is normalized into ascii quotes. 
 * </ol> 
 * 
 * The normalization performed by the four options above are (in the order in which
 * they appear in the @link Alpino specification):
 * <ol> 
...
public class AlpinoTokenizer<T> extends AbstractTokenizer<T> {

  
  private Alpino jlexer;
  
  /**
   * Construct a new Tokenizer which uses the @link Alpino specification
   * 
   * 
   * @param breader Reader
   * @param tokenFactory The TokenFactory that will be invoked to convert
   *        each string extracted by the @link Alpino  into a @Token object
   * @param properties the properties to run the tokenizer
   * 
   */
  public AlpinoTokenizer(BufferedReader breader, TokenFactory tokenFactory, Properties properties) {
    jlexer = new Alpino(breader, tokenFactory, properties);
  }

  /**
   * It obtains the next token. This functions performs the actual tokenization 
   * by calling the @link Alpino yylex() function
   *
